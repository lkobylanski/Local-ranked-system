...
function addPlayer() {
    ...
    if (nameleng > 0) {
        ...
        var s = saveData();
        //alert(player1.toSource()); //displaying object for debug puprposes
    }
    else {
...
function sortByAll() {
    ...
    display = display = "LADDER RESULTS sorted by <b> ELO</b>:<br>\"<i>CIĘŻAR KORONY</i>\"<br><br><img src=\"img/crown.png\" alt=\"crown.png\" class=\"crown\"><br>";
    ...
    if (ddl == "Choose Player") {
        ...
        for (var i = 0; i <= plength; i++) {
            if (ddl == players[i].name) {
                players[i].wins = players[i].wins + win_val;
                players[i].kills = players[i].kills + kill_val;
                players[i].style = players[i].style + styl_val;
                players[i].wks = (players[i].wins * 5) + players[i].kills + players[i].style;
